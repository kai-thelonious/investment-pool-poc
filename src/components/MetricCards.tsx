...
export default function MetricCards({
  ...
  const userDividendsEarned = dividends.reduce((sum, d) => sum + Math.round(d.totalPayout * shareRatio), 0);
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 font-sans">
        ...
        <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)]`}>
          ...
          <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">{currentUserSharePercent}%</h2>
          <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
        ...
        <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)] sm:col-span-2 lg:col-span-1`}>
          ...
          <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">${userDividendsEarned.toLocaleString()}</h2>
          <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 font-sans">
        ...
        <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)]`}>
          ...
          <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">{users.length} LPs</h2>
          <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
        ...
        <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)] sm:col-span-2 lg:col-span-1`}>
          ...
          <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
            {pendingTxCount > 0 ? `${pendingTxCount} pending subscription request(s)` : 'Clearances queue fully settled'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 font-sans">
      ...
      <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)]`}>
        ...
        <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">${completedInflows.toLocaleString()}</h2>
        <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
      ...
      <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)]`}>
        ...
        <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">{transactions.length} Entries</h2>
        <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
      ...
      <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder} shadow-[0_1px_3px_rgba(0,0,0,0.02)] sm:col-span-2 lg:col-span-1`}>
        ...
        <h2 className="text-2xl sm:text-3xl font-normal mt-1 text-[#141413]">{pendingTxCount} Pending</h2>
        <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-2`}>
          {totalPendingDeposits > 0 ? `$${totalPendingDeposits.toLocaleString()} awaiting clearance` : 'No pending queue'}
        </p>
      </div>
    </div>
  );
