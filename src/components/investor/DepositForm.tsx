...
export default function DepositForm({
  ...
  return (
    <div
      className={`${kamiTheme.cardBg} p-5 sm:p-8 rounded-xl border ${kamiTheme.cardBorder} flex flex-col justify-between`}
    >
      <div>
        ...
        <form onSubmit={handleUserDeposit} className="space-y-4 font-sans">
          <div>
            <label
              className={`block text-[11px] sm:text-xs font-semibold uppercase tracking-wider ${kamiTheme.textMuted} mb-2`}
            >
              Subscription Amount ($)
            </label>
            <input
      ...
      {currentUser.pending > 0 && (
        <div
          className={`mt-4 sm:mt-6 p-3 rounded-lg border ${kamiTheme.accentLight} text-xs font-sans`}
        >
