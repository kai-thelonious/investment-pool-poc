      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mt-0.5`}>
            Historical yield distributions and {currentUser.name}&apos;s pro-rata allocation (
            {shareNum}%).
          </p>
        </div>
            ...
            {dividends.map((div) => {
              ...
              return (
                <tr key={div.id} className="hover:bg-[#F5F4ED]/50 transition-colors">
                  <td className="py-3 px-2.5 font-mono font-medium text-[#141413] whitespace-nowrap">
                    {div.quarter}
                  </td>
                  <td className="py-3 px-2.5 text-[#504E49] whitespace-nowrap">{div.payoutDate}</td>
                  <td className="py-3 px-2.5 font-semibold text-[#1B365D] whitespace-nowrap">
                    {div.yieldPercent}%
                  </td>
                  <td className="py-3 px-2.5 text-[#141413] whitespace-nowrap">
                    ${div.totalPayout.toLocaleString()}
                  </td>
                  <td className="py-3 px-2.5 font-bold text-[#141413] whitespace-nowrap">
