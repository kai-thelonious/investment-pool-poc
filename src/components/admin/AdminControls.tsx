...
export default function AdminControls({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Capital Clearances */}
      <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder}`}>
        <h3 className="font-normal text-base sm:text-lg mb-1 text-[#141413]">Capital Clearances</h3>
        <p className={`text-[11px] sm:text-xs font-sans ${kamiTheme.textSub} mb-4`}>Approve incoming investor subscriptions.</p>
        {totalPendingDeposits > 0 ? (
          <div className="font-sans space-y-3">
            <div className="text-xs p-3 bg-[#F5F4ED] border border-[#E8E6DC] rounded-lg">
      ...
      <div className={`${kamiTheme.cardBg} p-5 sm:p-6 rounded-xl border ${kamiTheme.cardBorder}`}>
        <h3 className="font-normal text-base sm:text-lg mb-1 text-[#141413]">Post Valuation Mark</h3>
        ...
        <form onSubmit={handleUpdateFundValue} className="space-y-3 font-sans">
          <input
            ...
            className={`w-full ${kamiTheme.inputBg} border ${kamiTheme.inputBorder} rounded-lg py-2.5 px-3 text-xs ${kamiTheme.textPrimary} focus:outline-none focus:border-[#1B365D]`}
          />
          <button
            type="submit"
